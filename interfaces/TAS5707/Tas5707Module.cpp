...
{
    T regVal = cep::VectorToVal<T, true>(i2c->ReceiveFrameFromRegister(addr, reg, sizeof(T)).data);
...
void Tas5707Module::SetBiquadFilters(const cep::Tas5707::BiquadBanks& ch1,
...
void Tas5707Module::SetDynamicRangeCtrl(const cep::Tas5707::DynamicRangeControl& drc)
{
    using namespace cep::Tas5707;
...
#    endif
    ...
    writeFunc(cep::ValToVector<true>(drc.T), Registers::DrcT);
    writeFunc(cep::ValToVector<true>(drc.K), Registers::DrcK);
    writeFunc(cep::ValToVector<true>(drc.O), Registers::DrcO);
    writeFunc(cep::ValToVector<true>(drc.State), Registers::DrcCtrl);
...
{
    CEP_ASSERT(cep::Tas5707::BankSwitchingModes::IsValid(mode), "Invalid bank switching mode!");

    if (!UpdateRegisterValue(m_hw.I2cHandle,
                             m_i2cAddr,
                             (uint8_t)cep::Tas5707::Registers::BankSwitchCtrl,
                             mode,
...
{
    CEP_ASSERT(cep::Tas5707::SerialDataMode::IsValid(in), "Invalid serial data interface!");

    if (!UpdateRegisterValue(
          m_hw.I2cHandle, m_i2cAddr, (uint8_t)cep::Tas5707::Registers::SerDataInter, in))
    {
...
void Tas5707Module::ToggleSoftMute(bool ch1, bool ch2)
{
    if (!UpdateRegisterValue(m_hw.I2cHandle,
                             m_i2cAddr,
                             (uint8_t)cep::Tas5707::Registers::SoftMute,
...
void Tas5707Module::SetMasterVolume(uint8_t vol)
{
    ...
    if (!UpdateRegisterValue(
          m_hw.I2cHandle, m_i2cAddr, (uint8_t)cep::Tas5707::Registers::MasterVol, rVal))
    {
...
void Tas5707Module::SetChannelVolume(cep::Tas5707::Channels ch, uint8_t vol)
{
    uint8_t rVal    = VolumeToReg(vol);
    uint8_t channel = (uint8_t)cep::Tas5707::Registers::MasterVol + (uint8_t)ch;
...
void Tas5707Module::SendConfiguration()
{
    ...
    SetChannelVolume(cep::Tas5707::Channels::Channel1, 0x9E);    // 0dB
    SetChannelVolume(cep::Tas5707::Channels::Channel2, 0x9E);    // 0dB
...
bool Tas5707Module::Enable()
{
    ...
    if (!UpdateRegisterValue(
          m_hw.I2cHandle, m_i2cAddr, (uint8_t)cep::Tas5707::Registers::SysCtrl2, (uint8_t)0x00))
    {
...
bool Tas5707Module::Disable()
{
    ...
    if (!UpdateRegisterValue(
          m_hw.I2cHandle, m_i2cAddr, (uint8_t)cep::Tas5707::Registers::SysCtrl2, (uint8_t)0x40))
    {
