...
{
    if (HAL_I2C_Master_Transmit(m_handle, addr, const_cast<uint8_t*>(data), 
        len, I2cModule::TIMEOUT) != HAL_OK)
    {
...
I2C::Frame I2cModule::ReceiveFrame(uint8_t addr, size_t len)
{
    ...
    if (HAL_I2C_Master_Receive(m_handle, addr, frame.data.data(), frame.data.size(), I2cModule::TIMEOUT) != HAL_OK)
    {
...
I2C::Frame I2cModule::ReceiveFrameFromRegister(uint8_t addr, 
    uint8_t regAddr, 
    size_t len)
{
    I2C::Frame frame;
