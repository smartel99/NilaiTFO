...
#define LABEL_TO_STRING(x) #x
    ...
    std::uint8_t countOfOnesInBytesInator(std::uint8_t *bytes,
                                          std::uint8_t len);

    bool plus_minus(std::int32_t value, std::int32_t compare,
                    std::int32_t margin);
    bool plus_minus(double value, double compare, double margin);

    size_t FindStringInVector(const std::string &str,
